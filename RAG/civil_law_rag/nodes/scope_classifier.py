...
def _chapter_threshold() -> float:
    return float(
        cfg.get("rag", {}).get("civil_law", {}).get("scope_chapter_threshold", 0.5)
    )


def _section_threshold() -> float:
    return float(
        cfg.get("rag", {}).get("civil_law", {}).get("scope_section_threshold", 0.5)
    )


# ---------------------------------------------------------------------------
# Formatters — build the enumerated lists fed to the LLM
# ---------------------------------------------------------------------------
...
def _classify_chapter(query: str, state: dict) -> Optional[dict]:
    """Returns {chapter_id, confidence, reason} or None on failure."""
    ...
    try:
        ...
        data = json.loads(_strip_fences(resp.content.strip()))
        return data
...
def _classify_section(query: str, chapter: dict, state: dict) -> Optional[dict]:
    """Returns {section_id, confidence, reason} or None on failure."""
    if state.get("llm_call_count", 0) >= MAX_LLM_CALLS:
...
def scope_classifier_node(state: dict) -> dict:
    ...
    # ── Stage 1: chapter ─────────────────────────────────────────────────────
    ch_result = _classify_chapter(query, state)

    if not ch_result or ch_result.get("confidence", 0) < ch_thresh:
        log_event(
            logger, "scope_no_filter",
            query=query,
    ...
    # ── Stage 2: section ─────────────────────────────────────────────────────
    sc_result = _classify_section(query, chapter, state)
    ...
    if sc_result and sc_result.get("confidence", 0) >= sc_thresh:
        ...
        if section:
            ...
            log_event(
                logger, "scope_chapter_and_section",
                chapter=chapter["title"],
                section=section["title"],
                ch_confidence=ch_conf,
                sc_confidence=state["scope_confidence"],
            )
            return state
