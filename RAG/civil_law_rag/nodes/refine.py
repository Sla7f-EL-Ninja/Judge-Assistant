...
MAX_LLM_CALLS: int = 5
LLM_TIMEOUT: int = 30
from RAG.civil_law_rag.prompts import UNIFIED_REFINE_PROMPT
...
def refine_node(state: dict) -> dict:
    ...
    # Budget guard: if no LLM budget left, keep current query unchanged
    ...
    try:
        ...
        # If null: keep previous refined/rewritten query → retry with same wording
        log_event(
            logger, "refine",
