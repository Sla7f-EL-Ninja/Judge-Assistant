...
# eval_harness.py
# ---------------
# Evaluation harness for the Unified Legal RAG pipeline.

# Metrics:
#   - corpus_routing_accuracy:   fraction of queries routed to the correct legal corpus
#   - retrieval_precision_at_5:  fraction of expected articles in top-5
#   - citation_faithfulness:     fraction of cited article numbers that were retrieved
#   - off_topic_accuracy:        fraction of off-topic queries correctly rejected
#   - avg_llm_calls:             mean LLM calls per query (budget health)
...
#         sid     = sample["id"]
#         query   = sample["query"]
#         expected_corpus = sample.get("expected_corpus", "none")
#         expected_articles = set(sample.get("expected_articles", []))
#         should_fail       = sample.get("should_fail", False)
...
#         # 1. Corpus Routing Accuracy
#         if expected_corpus != "none" and not should_fail:
#             routing_total += 1
#             actual_corpus = rs.get("corpus")
...
#     metrics: Dict[str, Any] = {
#         "total_samples":              total,
#         "corpus_routing_accuracy":    (routing_correct / routing_total if routing_total else None),
#         "retrieval_precision_at_5":   (sum(retrieval_precisions) / len(retrieval_precisions) if retrieval_precisions else None),
#         "citation_faithfulness":      (sum(citation_faithfulnesses) / len(citation_faithfulnesses) if citation_faithfulnesses else None),
#         "off_topic_accuracy":         (off_topic_correct / off_topic_total if off_topic_total else None),
#         "avg_llm_calls":              sum(llm_calls) / len(llm_calls) if llm_calls else 0,
#         "avg_latency_ms":             sum(latencies) / len(latencies) if latencies else 0,
#         "failures":                   failures,
#     }
...
# @pytest.mark.eval
# def test_corpus_routing_accuracy(eval_metrics):
#     score = eval_metrics["corpus_routing_accuracy"]
#     if score is None:
#         pytest.skip("No samples with expected_corpus")
#     assert score >= _THRESHOLDS["corpus_routing_accuracy"], (
#         f"Corpus routing accuracy = {score:.3f} < threshold {_THRESHOLDS['corpus_routing_accuracy']}\n"
#     )
...
# Path setup to ensure RAG module is importable
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../..")))

# Pointing to the unified legal_rag structure[cite: 23]
from RAG.legal_rag.graph import build_graph
from RAG.legal_rag.state import make_initial_state
...
def _retrieved_indices(result_state: dict) -> List[int]:
    """Extract article indices from retrieval metadata."""
    return [
        d.metadata.get("index")
        for d in result_state.get("last_results", [])
        if d.metadata.get("index") is not None
...
def _cited_indices(answer: str) -> List[int]:
    """Extract article numbers mentioned in the LLM answer."""
    return [int(m) for m in re.findall(r"المادة\s+(\d+)", answer or "")]


def _is_terminal_failure(result_state: dict) -> bool:
    """True if the pipeline reached a fallback state."""
    answer = result_state.get("final_answer", "")
    classification = result_state.get("classification", "")
    return (
        classification == "off_topic"
...
def run_eval() -> Dict[str, Any]:
    ...
    for sample in samples:
        ...
        # 1. Corpus Routing Accuracy[cite: 23]
        if expected_corpus != "none" and not should_fail:
            routing_total += 1
            actual_corpus = rs.get("corpus")
            if actual_corpus == expected_corpus:
        ...
        # 3. Retrieval precision@5[cite: 23]
        if expected_articles:
            retrieved = set(_retrieved_indices(rs))
            hit = len(expected_articles & retrieved)
            precision = hit / len(expected_articles)
        ...
        # 4. Citation faithfulness[cite: 23]
        retrieved_all = set(_retrieved_indices(rs))
        cited         = set(_cited_indices(rs.get("final_answer", "")))
        if cited:
            faithful_count = len(cited & retrieved_all)
