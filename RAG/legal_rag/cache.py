...
cache.py
--------
Versioned semantic response cache for the legal_rag engine.
...
Versioning values are taken from CorpusConfig so each corpus has
independent cache invalidation.  The cache is in-memory (lost on
restart) — intentional for a legal system where staleness carries risk.
TTL-based eviction is not implemented; entries are evicted FIFO when
max_size is reached.
"""
...
def _cache_key_prefix(
    ...
    raw = (
        f"{query}"
        f"|{corpus_config.prompts_version}"
        f"|{corpus_config.corpus_version}"
...
class SemanticCache:
    """In-memory semantic similarity cache with per-corpus versioned keys."""
    ...
    def _get_embeddings(self):
        if self._embeddings is None:
            from RAG.legal_rag.retrieval.embeddings import get_client
    ...
    def get(
        ...
        key_prefix = _cache_key_prefix(query, corpus_config, llm_model)
