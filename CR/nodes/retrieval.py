...
def _deduplicate_articles(articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Deduplicate by article_number, keeping first occurrence."""
    seen = set()
    result = []
    for art in articles:
...
def retrieve_law_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Fallback to generic query if no per-element queries available
    if not law_queries:
        source_text = state.get("source_text", "")
    ...
    for item in law_queries:
        ...
        try:
            ...
            last_result = result  # keep last successful result for answer field
            logger.debug(
                "Law retrieval [%s] '%s': %d articles", element_id, issue_title, len(parsed)
            )
        except Exception as exc:
    ...
    logger.info(
        "Law retrieval for '%s': %d total articles (%d unique) across %d queries",
...
def retrieve_facts_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item in fact_queries:
        ...
        try:
            ...
            if result.get("error"):
                logger.warning(
                    "retrieve_facts_node [%s] partial error for '%s': %s",
                    element_id, issue_title, result["error"],
                )
            if answer.strip():
                # Label each element's facts so downstream LLMs can distinguish them
                per_element_facts.append(f"[{element_id}]:\n{answer.strip()}")
            logger.debug(
                "Fact retrieval [%s] '%s': %d chars", element_id, issue_title, len(answer)
            )
        except Exception as exc:
    ...
    # Combine into a single labeled string — backward compatible with all downstream nodes
    retrieved_facts = "\n\n".join(per_element_facts) if per_element_facts else ""

    logger.info(
        "Fact retrieval for '%s': %d elements answered, %d chars total",
