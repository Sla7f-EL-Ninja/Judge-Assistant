...
#     def test_total_key_count(self):
#         assert len(IssueAnalysisState.__annotations__) == 22
...
#         expected = {
#             "decompose_issue", "retrieve_law", "retrieve_facts",
...
#         state = {"case_id": "test-case-001"}  # no identified_issues key
#         result = issue_dispatch_router(state)
#         assert result == "generate_empty_report"
...
#         ]
#         result = issue_dispatch_router(self._make_state(issues))
#         # Modifying one payload should not affect the other
...
#         issues = [
#             {"issue_id": i, "issue_title": f"مسألة {i}", "legal_domain": "عقود", "source_text": "نص"}
#             for i in range(1, 10)  # 9 issues, above warning threshold of 8
#         ]
